...
class Card:
    ...
    def validCards(hand, trump, trickSoFar):
        ...
        if requestedSuit != trump:
            ...
            if not notYetPlayed[positionAlly] and Card.trick(trickSoFar, leadingPlayer, trump) == positionAlly:
                cardsValid = [card for card in hand if card.suit != trump or card.orderForTrickTaking(trump = True) > trickSoFar[positionAlly].orderForTrickTaking(trump=trickSoFar[positionAlly].suit==trump)]
                if cardsValid:
                    return cardsValid
                else:
                    return hand
            cardsValid = [card for card in hand if card.suit == trump and ((not trumpsInTrick) or card.orderForTrickTaking(trump=True) >  highesttrumpInTrick.orderForTrickTaking(trump=True))]
            if cardsValid:
                return cardsValid
            ...
            cardsValid = [x for x in hand if x.suit == trump and x.orderForTrickTaking(trump=True) >  highesttrumpInTrick.orderForTrickTaking(trump = True)]
            if cardsValid:
                return cardsValid
    ...
    def trick(cards, leadingPlayer, trump): 
         highest = [(card.orderForTrickTaking(cards[leadingPlayer].suit, card.suit == trump),indi) for indi, card in enumerate(cards) if type(card) is Card]
         _,maxi =  max(highest)
         return maxi
    ...
    def roem(cards,trump):
        ...
        for suit in Card.suits:
             ...
             if diffList.count(1) == 2 and np.diff([ind for ind, difference in enumerate(diffList) if difference == 1]).tolist() == [1]:
                 return 20
    ...
    def updatePossibilities(trickSoFar, trump, cardPlayed, activePlayer, possibilities):
        allCards = [Card(number, suit) for number in range(7, 15) for suit in Card.suits]
        trickWithoutLastCard = [maybeCard for maybeCard in trickSoFar]
        trickWithoutLastCard[activePlayer] = 0
        maybePlayable = [card for card in allCards if possibilities[activePlayer][card.toindex()] == 1]
        notPlayable = []
...
class OneDeal:
    ...
    def __init__(self, botsSettings = 0,activePlayer = 0):
        ...
        self.determineTrump()
        self.log = []
        # self.playerStrat = [partial(OneDeal.randomBot,trump = self.trump)]*4\
        if botsSettings == 0:
            ...
            for indi, setting in enumerate(botsSettings):
                self.playerStrat[indi] = partial(mikeBot,trump = self.trump, numberItsDistri=setting[0], numberItsTreesearch=setting[1])
            self.trump = mikeBotDetermineTrump(self.players[activePlayer], activePlayer,botsSettings[activePlayer][0]//4,botsSettings[activePlayer][1])
        
        # self.playerStrat[0] = partial(OneDeal.mikeBot,trump = self.trump)
        ...
        for i in range(8):
            ...
            for j in range(4):
                ...
                if not self.players[whoNext]:
                    continue                   
                trick[whoNext]= self.playerStrat[whoNext](self.players[whoNext],trick,record,whoNext,self.activePlayer,possibilities)
                self.players[whoNext].remove(trick[whoNext])
                Card.updatePossibilities(trick, self.trump, trick[whoNext], whoNext, possibilities)
            ...
            print (i+1)
        #OneDeal.gameRecord(record,self.trump)
        self.log = record
    ...
    def shuffleDeck(self):
        self.deck = [Card(number, suit) for number in range(7, 15) for suit in Card.suits]
        self.deck = np.random.permutation(self.deck).tolist()
        self.players[0] = self.deck[0:8]
        self.players[1] = self.deck[8:16]
        self.players[2] = self.deck[16:24]
        self.players[3] = self.deck[24:32]
    ...
    def naiveValidCards(hand, log,playerID,trickSoFar):
        ...
        filteredCards = [card for card in allCards if not (card in hand or card in cardsInTrick or card in [card for card in trickSoFar if type(card) is Card])]
        shuffledCards = np.random.permutation(filteredCards).tolist()
        toWhom = playerID
        # shuffledCards[0].printCard()
    ...
    def forcedCardsFromPos(possibilities):
        ...
        for card in allCards:
            if possibilities[:,card.toindex()].sum == 1:
                forcedCards.append(card,possibilities[:][card.toindex()].index(1))
        return forcedCards
    ...
    def divideCards(hand, log,playerID,trickSoFar,possibilities):
        ...
        possibilitiesUpdated[:,[card.toindex() for card in hand]]         #remove player hand from COPY of possibilities
        ...
        filteredCards = [card for card in allCards if not (card in hand or card in cardsSoFar or card in cardsInTrick)]
        ...
        for (card, whichPlayer) in forcedCards:
            players[whichPlayer].append(card)
            filteredCards.remove(card)
            cardsPerPlayer[whichPlayer]+= -1
...
def mikeBot(hand,trickSoFar,log,playerID,leadingPlayer,possibilities,trump,numberItsDistri=100, numberItsTreesearch=100):
    resultList = []
    possibleCards = Card.validCards(hand, trump, trickSoFar)
    values = [0]*len(possibleCards)
    
    for _ in range(numberItsDistri):
        resultList.append(doSimulation(hand,trickSoFar,log,playerID,leadingPlayer,trump,possibilities,numberItsTreesearch)) 
...
def mikeBotDetermineTrump(hand, playerID, numberItsDistri=100, numberItsTreesearch=100):
    resultList = []
    values = [0]*4
    trickSoFar = [0]*4
    for _ in range(numberItsDistri):
